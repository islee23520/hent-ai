...
- [채널 필터 (blocklist / allowlist)](#채널-필터-blocklist--allowlist)
...
```jsonc
{
  "plugins": {
    "emotion-image": {
      ...
      // 채널 필터
      "channels": {
        "mode": "blocklist",   // "blocklist" 또는 "allowlist"
        "list": ["9999999999"] // 제외/허용할 채널 ID 목록
      },
...
## 채널 필터 (blocklist / allowlist)

특정 채널에서 감정 이미지를 활성/비활성화합니다.
...
```
→ `list`에 있는 채널만 이미지 활성, 나머지는 비활성

### 판별 로직

```typescript
// index.ts의 isChannelEnabled 함수
function isChannelEnabled(channelId: string): boolean {
  if (channelList.size === 0) return true;  // 목록 비어있으면 전부 활성
  if (channelMode === "allowlist") return channelList.has(channelId);
  return !channelList.has(channelId);  // blocklist
}
```

> **참고:** 채널 ID는 `channel:` 접두사 없이 순수 숫자 snowflake만 사용합니다. 코드가 `channel:` 접두사를 자동으로 제거합니다.
...
DB 프로필 매핑이 **우선**입니다. 프로필 매핑이 없는 채널에서만 channel-overrides가 참조됩니다.
...
> **권장:** 새 프로필은 DB 기반 시스템을 사용하세요. channel-overrides는 하위 호환성을 위해 유지됩니다.
...
1. 사용자 메시지 수신
2. `isChannelEnabled(channelId)` 확인
...
6. **channel-overrides vs DB** — 두 시스템이 공존하지만 DB 매핑이 우선입니다. 혼란을 줄이려면 하나만 사용하세요.
